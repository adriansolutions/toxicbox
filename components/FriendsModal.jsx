      ...
      <div className="w-full max-w-md rounded-3xl bg-white dark:bg-[#1e1f22] border border-white/10 shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-5 border-b flex justify-between items-center">
          <div>
            ...
            <p className="text-sm opacity-60">Add and accept friends</p>
          </div>

          <button onClick={close} className="w-10 h-10 rounded-xl bg-black/10">
            ✕
          </button>
        </div>
        ...
        <div className="p-5">
            ...
            {results.map((user) => (
              <div key={user.userId} className="flex items-center gap-3 p-3 rounded-2xl bg-black/5 dark:bg-white/5">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-sm opacity-60">{user.userId}</div>
                </div>

                <button
                  onClick={() => addFriend(user)}
                  className="px-4 h-10 rounded-xl bg-blue-600 text-white font-bold"
                >
                  Add
                </button>
              </div>
          ...
          {/* REQUESTS */}
          <div className="mt-8">
            <div className="font-bold mb-3">Incoming Friend Requests</div>

            <div className="space-y-3">
              {incomingRequests.length === 0 && (
                <div className="text-sm opacity-60">No requests</div>
              )}
              ...
              {incomingRequests.map((user) => (
                <div key={user.userId} className="flex items-center gap-3 p-3 rounded-2xl bg-black/5 dark:bg-white/5">
                  ...
                  <button
                    ...
                    className="px-4 h-10 rounded-xl bg-green-600 text-white font-bold"
                  >
                    Accept
                  </button>
